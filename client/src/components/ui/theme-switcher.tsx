...
type TTheme = ThemePreference;

export const ThemeSwitcher = () => {
  const { theme, setTheme } = useTheme();
  const id = useId();

  const optionClass = (value: TTheme, extraWhenActive: string) =>
    `flex items-center justify-center h-8 w-8 cursor-pointer rounded-[999999px] group${
      theme === value ? extraWhenActive : ""
    }`;

  const fillClass = (value: TTheme) =>
    `group-hover:fill-gray-1000 duration-150 ${theme === value ? "fill-gray-1000" : "fill-gray-700"}`;

  return (
    <fieldset
      className="m-0 flex h-8 rounded-[999999px] border border-shadow p-0"
    ...
    >
      ...
      <div className="ml-[-1px] mt-[-1px]">
        ...
        />
        <label htmlFor={`${id}-theme-switch-system`} className={optionClass("system", " border-r border-shadow")}>
          <span className="sr-only">system</span>
          <svg height="16" strokeLinejoin="round" viewBox="0 0 16 16" width="16">
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              className={fillClass("system")}
      ...
      <div className="mt-[-1px]">
        <input
          ...
          className="absolute m-0 appearance-none p-0"
        />
        <label htmlFor={`${id}-theme-switch-light`} className={optionClass("light", " border border-shadow")}>
      ...
      <div className="mr-[-1px] mt-[-1px]">
        <input
          ...
          className="absolute m-0 appearance-none p-0"
        />
        <label htmlFor={`${id}-theme-switch-dark`} className={optionClass("dark", " border-l border-shadow")}>
