 ...
 * Handles ## Case 1, **Case 2**, "Case 3:", "Case IV Begins: …", etc.
 */
function normalizeOralBoardCaseHeaders(text: string): string {
  ...
  for (const line of lines) {
    ...
    s = s.replace(/^#{1,6}\s+/, '');
    s = s.replace(/^\*{1,2}\s*/, '').replace(/\s*\*{1,2}$/, '');
    s = s.trim();
    const m = s.match(/^case\s+(\d+|[IVXLCDM]+)(\s+begins)?\b(.*)$/i);
    if (!m) {
      out.push(line);
      continue;
    }
...
export function OralBoardAssistantContent({
  ...
  const cleaned = normalizeOralBoardCaseHeaders(stripForOralBoardDisplay(content));
